...
def draw_pipeline():
    ...
    # Phase 1: BUILD
    # =========================================================================
    phase_bg(11.0, 8.5, "PHASE 1: BUILD (before Selection Sunday)")
    ...
    # Row 2: Feature Engineering — centered
    fey = 16.5
    box(2.5, fey, 9, 1.1, "Feature Engineering\n18 matchup features (team A − team B differences)\n~6,000 training rows across 12 seasons", C_PROCESS, 8.5)

    # Arrows: data sources → feature engineering (vertical, centered on each box)
    for bx in [0.7, 3.9, 7.1, 10.3]:
        cx = bx + dsw / 2
        arrow_v(cx, dsy, fey + 1.1)

    # Row 3: Model Training — centered
    mty = 14.6
    box(2.5, mty, 9, 1.2, "Model Training (LOSO Cross-Validation)\nStacked Ensemble: 6 base models → logistic meta-learner\nLeave-one-season-out prevents overfitting", C_MODEL, 8.5)

    arrow_v(7, fey, mty + 1.2)

    # Row 4: Calibration / Evaluation / Output — three boxes
    r4y = 12.4
    box(0.7, r4y, 3.5, 0.9, "Calibration\nIsotonic / Platt / Temperature", C_MODEL, 8.5)
    box(5.2, r4y, 3.5, 0.9, "Evaluation\nLOSO CV, ECE, seed-tier", C_PROCESS, 8.5)
    box(9.7, r4y, 3.5, 0.9, "models/saved/model.pkl\n(trained model)", C_OUTPUT, 8.5)

    # Arrows: training → three outputs
    ...
    # Phase 2: OPTIMIZE
    # =========================================================================
    phase_bg(0.3, 10.3, "PHASE 2: OPTIMIZE (after Selection Sunday)")

    # Row 5: Bracket + Predictor + KenPom
    r5y = 9.4
    box(0.7, r5y, 3, 0.85, "bracket.json\n(64 teams)", C_DATA, 8.5)
    box(4.2, r5y, 5, 0.85, "Predictor (3-tier fallback)\nML model → KenPom → Seed-based", C_MODEL, 8.5)
    box(9.7, r5y, 3.5, 0.85, "kenpom_2026.csv\n(current season)", C_DATA, 8.5)

    # Arrows into predictor
    arrow_h(r5y + 0.42, 3.7, 4.2)   # bracket → predictor
    arrow_h(r5y + 0.42, 9.7, 9.2)   # kenpom → predictor (right to left)

    # model.pkl → predictor (vertical down, slight angle)
    arrow_v(11.45, r4y, r5y + 0.85)

    # Row 6: Monte Carlo Simulation
    r6y = 7.7
    box(3.5, r6y, 7, 1, "Monte Carlo Simulation\n50,000 full-bracket simulations\nP(team reaches round R)", C_PROCESS, 8.5)

    arrow_v(7, r5y, r6y + 1)

    # Row 7: Optimization components — 4 purple boxes with equal width
    r7y = 5.5
    ow = 3.0
    oh = 1.3
    box(0.7, r7y, ow, oh, "Ownership Model\nNash equilibrium\n+ heuristic blend\n+ brand/recency bias", C_OPT, 7.5)
    box(4.0, r7y, ow, oh, "DP Future Values\nBackward induction\n9 contest days\nSave top seeds for later", C_OPT, 7.5)
    box(7.3, r7y, ow, oh, "Analytical EV\nExact closed-form\nEV per pick", C_OPT, 7.5)
    box(10.6, r7y, 2.7, oh, "Portfolio Search\nGreedy + swap (hybrid)\nor ACO", C_OPT, 7.5)

    # MC Simulation → Ownership, DP, Analytical (fan out vertically)
    arrow_diag(5.5, r6y, 2.2, r7y + oh)
    arrow_v(5.5, r6y, r7y + oh)
    arrow_diag(8.5, r6y, 8.8, r7y + oh)

    # Horizontal flow: Ownership → Portfolio, DP → Portfolio, Analytical → Portfolio
    # Stagger vertically so arrows don't overlap
    arrow_h(r7y + oh * 0.65, 3.7, 10.6)   # ownership → portfolio (upper)
    arrow_h(r7y + oh * 0.5, 7.0, 10.6)    # DP → portfolio (middle)
    arrow_h(r7y + oh * 0.35, 10.3, 10.6)  # analytical → portfolio (lower)

    # Row 8: Pick Recommendations
    r8y = 3.5
    box(2.5, r8y, 9, 1.3, "Pick Recommendations\nEntry 1: (3) Iowa St. + (3) Purdue    Win 93%  Own 2%\nEntry 2: (4) Kansas + (2) UConn        Win 89%  Own 1.7%\nTotal EV: $663  |  Joint survival: 100%", C_OUTPUT, 8)

    arrow_v(11.9, r7y, r8y + 1.3)

    # Row 9: Tournament tracking — three boxes
    r9y = 1.3
    box(0.7, r9y, 3.2, 0.9, "Record Results\nmarchmadness results\n--day N", C_PROCESS, 8.5)
    box(4.5, r9y, 3.2, 0.9, "Entry Manager\nTrack alive/eliminated\nEnforce no-reuse", C_PROCESS, 8.5)
    box(8.3, r9y, 3.2, 0.9, "Next Day\nRe-optimize with\nupdated bracket", C_PROCESS, 8.5)

    # Recommendations → tracking row
    arrow_diag(5, r8y, 2.3, r9y + 0.9)
    arrow_v(6.1, r8y, r9y + 0.9)

    # Horizontal flow: Record → Entry Manager → Next Day
    arrow_h(r9y + 0.45, 3.9, 4.5)
    arrow_h(r9y + 0.45, 7.7, 8.3)

    # Loop-back: Next Day → back to Predictor row (dashed arc on right side)
    ax.annotate(
        "", xy=(12.8, r5y + 0.42), xytext=(11.5, r9y + 0.45),
        arrowprops=dict(
            arrowstyle="-|>", color=C_ARROW, lw=1.3,
            connectionstyle="arc3,rad=-0.4", linestyle="dashed",
        ),
    )
    ax.text(13.0, 5.2, "repeat\neach\ngame day",
    ...
    # Title
    # =========================================================================
    ax.text(7, 19.6, "March Madness Survivor Pool Optimizer — Pipeline",
